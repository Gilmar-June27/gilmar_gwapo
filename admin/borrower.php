...
if (isset($_POST['add_borrower'])) {
    ...
    $insert = "INSERT INTO borrower (collector_id, tax_id) VALUES ('$collector_id', '$tax_id')";
...
if (isset($_POST['update_borrower'])) {
    ...
    $update_query = "UPDATE borrower SET collector_id = '$collector_id', tax_id = '$tax_id' WHERE borrower_id = $borrower_id";
...
<div class="az-content-breadcrumb">
            <span>Junk</span>
            <span>Collector Borrower</span>           
          </div>
          <h2 class="az-content-title">Collector Borrower</h2>
    <button class="btn add-btn" id="openBorrowerModal">Add Borrower</button>
        ...
        <div class="modal-content">
            ...
            <form method="POST"> <!-- Changed method to POST -->
                <div class="mb-3">
                    ...
                    <select class="form-control" name="collector_id" id="collector_id" required>
                        <option value="" disabled selected>-- Choose Collector --</option>
                        <?php
                        $collectors = mysqli_query($conn, "SELECT id, first_name, last_name FROM users WHERE user_type = 'collector'");
                        while ($row = mysqli_fetch_assoc($collectors)) {
    ...
    <div class="modal-content">
        ...
        <form action="" method="POST">
            ...
            <div class="mb-3">
                <label for="collector_id" class="form-label">Select Collector</label>
                <select class="form-control" name="collector_id" id="editCollectorId" disabled>
...
<script>
function openEditModal(borrower_id, tax_id, collector_name) {
    document.getElementById('editBorrowerId').value = borrower_id;
    document.getElementById('editTaxId').value = tax_id;
    document.getElementById('editCollectorId').value = collector_name;
