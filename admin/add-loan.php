...
if (isset($_POST['apply_loan'])) {
  ...
  if ($borrower_result && mysqli_num_rows($borrower_result) > 0) {
      ...
      $money_query = "SELECT * FROM total_money WHERE admin_id = '$admin_id' ORDER BY id DESC LIMIT 1";
      ...
      if ($money_result && mysqli_num_rows($money_result) > 0) {
          ...
          $prev_deductions = floatval($money_row['deduction_of_capital_money']);

          // ✅ Check if admin has enough capital
          if ($amount > ($capital_money - $prev_deductions)) {
              echo "<script>alert('Not enough capital available from admin.'); window.location.href='';</script>";
              exit;
          }

          $new_deductions = $prev_deductions + $amount;
          $new_total_money = $capital_money - $new_deductions;

           // ✅ Insert new capital deduction row into total_money
       ...
       if (!mysqli_query($conn, $insert_money_query)) {
           echo "<script>alert('Error inserting capital deduction.'); window.location.href='';</script>";
           exit;
       }
          ...
          if (mysqli_query($conn, $insert_query)) {
              ...
              echo "<script>alert('Error inserting loan.'); window.location.href='';</script>";
          }

      } else {
          echo "<script>alert('No capital money found for this admin.'); window.location.href='';</script>";
      }
  } else {
...
<div style="text-align: right; margin-bottom: 10px;">
   <a href="add-loan.php"  class="btn btn-primary" onclick="filterLoans('pending')">All</a>
...
LIMIT $limit OFFSET $offset";
  ...
  <p><small>Address: <strong>' . htmlspecialchars($row['collector_address']) . '</strong></small></p>
          ...
          <button style="padding: 5px 10px;" class="btn edit-btn" data-toggle="modal" data-target="#editModal'.$row['loan_id'].'">Edit</button>
    ...
    <div class="modal fade" id="editModal' . $row['loan_id'] . '" style="z-index:50000;">
    ...
    <div class="modal-body">
        ...
        <input type="text" class="form-control" name="ref_no" value="' . $row['ref_no'] . '" readonly>
        ...
        <div class="row">
            <div class="col-md-6 mb-3">
                <label for="amount">Amount</label>
                <input type="number" class="form-control" name="amount" id="loanAmount' . $row['loan_id'] . '" value="' . $row['amount'] . '" required>
            </div>
            <div class="col-md-6 mb-3">
                <label for="lplan_id">Loan Plan</label>
                <select name="lplan_id" id="loanPlanSelect' . $row['loan_id'] . '" class="form-control" required>';

                $plans = mysqli_query($conn, "SELECT * FROM loan_plan");
                while ($plan = mysqli_fetch_assoc($plans)) {
                    $selected = $row['lplan_id'] == $plan['lplan_id'] ? 'selected' : '';
                    echo "<option value='{$plan['lplan_id']}' $selected data-months='{$plan['lplan_month']}' data-interest='{$plan['lplan_interest']}' data-penalty='{$plan['lplan_penalty']}'>
                            {$plan['lplan_month']} months | Interest: {$plan['lplan_interest']}% | Penalty: {$plan['lplan_penalty']}%
                          </option>";
                }
...
echo '
            ...
            <div class="col-md-6 mb-3">
                <label for="purpose">Loan Purpose</label>
                <textarea class="form-control" name="purpose" rows="4" required>' . $row['purpose'] . '</textarea>
            </div>
        </div>

        <button type="button" class="btn btn-info" onclick="calculateLoans(' . $row['loan_id'] . ')">Calculate</button>

        <div id="loanSummarys' . $row['loan_id'] . '" style="margin-top: 15px;">
            <p><strong>Total Payable:</strong> ₱<span id="totalPayables' . $row['loan_id'] . '">0.00</span></p>
            <p><strong>Monthly Payment:</strong> ₱<span id="monthlyPayables' . $row['loan_id'] . '">0.00</span></p>
            <p><strong>Penalty Amount:</strong> ₱<span id="penaltyAmounts' . $row['loan_id'] . '">0.00</span></p>
        </div>

        <input type="hidden" name="status" value="' . $row['status'] . '">
        <input type="hidden" name="date_created" value="' . $row['date_created'] . '">
        <input type="hidden" name="date_released" value="' . $row['date_released'] . '">
    </div>
    <div class="modal-footer">
        <button type="submit" name="update_loan" class="btn btn-primary">Update</button>
...
<script>
function calculateLoans(loanId) {
    const amountInput = document.getElementById('loanAmount' + loanId);
    const planSelect = document.getElementById('loanPlanSelect' + loanId);

    const amount = parseFloat(amountInput.value);
    const months = parseInt(planSelect.options[planSelect.selectedIndex].getAttribute('data-months'));
    const interestRate = parseFloat(planSelect.options[planSelect.selectedIndex].getAttribute('data-interest'));
    const penaltyRate = parseFloat(planSelect.options[planSelect.selectedIndex].getAttribute('data-penalty'));

    if (isNaN(amount) || isNaN(months) || isNaN(interestRate) || isNaN(penaltyRate)) {
        alert("Invalid loan amount or plan details.");
        return;
    }
    ...
    document.getElementById('totalPayables' + loanId).textContent = totalPayable.toFixed(2);
    document.getElementById('monthlyPayables' + loanId).textContent = monthlyPayment.toFixed(2);
    document.getElementById('penaltyAmounts' + loanId).textContent = penaltyAmount.toFixed(2);
}
