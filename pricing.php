        ...
        if (isset($_FILES['image']) && $_FILES['image']['error'] == 0) {
            ...
            if ($image_size > 50000000) {
                ...
                if (move_uploaded_file($image_tmp_name, $image_folder)) {
                    $query = "INSERT INTO `junk_price` (garbage_price, kl, image, junk_type, collector_id) 
                              VALUES('$garbage_price', '$kl', '$image_name', '$junk_type', '$collector_id')";
...
if (isset($_POST['update_product'])) {
    ...
    $update_sql = "UPDATE `junk_price` SET 
                    ...
                    kl = '$update_kg'
                    $image_query
                  WHERE id = '$update_id' AND collector_id = '$collector_id'";
...
$result = mysqli_query($conn, "SELECT * FROM `junk_price` WHERE `collector_id` = '$collector_id'");
...
// Count total records
$count_sql = "SELECT COUNT(*) as total FROM  junk_price WHERE collector_id='$collector_id'";
if ($search != '') {
...
$query = "SELECT junk_price.*, users.first_name, users.last_name, users.address
          FROM junk_price
          LEFT JOIN users ON junk_price.collector_id = users.id
          WHERE junk_price.collector_id='$collector_id'";
